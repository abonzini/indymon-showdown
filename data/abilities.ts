...
export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
	...
	chlorophyll: {
		onModifySpe(spe, pokemon) {
			if (['sunnyday', 'desolateland', 'continuousdesolateland'].includes(pokemon.effectiveWeather())) {
	...
	dryskin: {
		...
		onWeather(target, source, effect) {
			...
			if (effect.id === 'raindance' || effect.id === 'continuousrain' || effect.id === 'primordialsea' || effect.id === 'continuousprimordialsea') {
	...
	harvest: {
		...
		onResidualSubOrder: 2,
		onResidual(pokemon) {
			if (this.field.isWeather(['sunnyday', 'desolateland', 'continuousdesolateland']) || this.randomChance(1, 2)) {
	...
	hydration: {
		...
		onResidualSubOrder: 3,
		onResidual(pokemon) {
			if (pokemon.status && ['raindance', 'continuousrain', 'primordialsea', 'continuousprimordialsea'].includes(pokemon.effectiveWeather())) {
	...
	icebody: {
		...
		onImmunity(type, pokemon) {
			if (type === 'hail' || type === 'continuoushail') return false;
		},
		flags: {},
		name: "Ice Body",
		rating: 1,
	...
	leafguard: {
		onSetStatus(status, target, source, effect) {
			if (['sunnyday', 'desolateland', 'continuousdesolateland'].includes(target.effectiveWeather())) {
				if ((effect as Move)?.status) {
		...
		onTryAddVolatile(status, target) {
			if (status.id === 'yawn' && ['sunnyday', 'desolateland', 'continuousdesolateland'].includes(target.effectiveWeather())) {
				this.add('-immune', target, '[from] ability: Leaf Guard');
				return null;
			}
		},
	...
	solarpower: {
		...
		onWeather(target, source, effect) {
			if (target.hasItem('utilityumbrella')) return;
			if (effect.id === 'sunnyday' || effect.id === 'desolateland' || effect.id === 'continuousdesolateland') {
				this.damage(target.baseMaxhp / 8, target, target);
			}
		},
		flags: {},
	...
	swiftswim: {
		onModifySpe(spe, pokemon) {
			if (['raindance', 'continuousrain', 'primordialsea', 'continuousprimordialsea'].includes(pokemon.effectiveWeather())) {
				return this.chainModify(2);
			}
		},
		flags: {},
